...
public class BaseArea : MonoBehaviour
{
    ...
    protected float baseAreaWidth;
    protected float baseAreaHeight;
    ...
    void Start()
    {
        baseAreaWidth = baseArea.GetComponent<RectTransform>().rect.width;
        baseAreaHeight = baseArea.GetComponent<RectTransform>().rect.height;
    }
    ...
    private void ReArrange()
    {
        ...
        float gap = 5f; // 左右gap
        bool tooManyCards = (cardWidth + gap) * cardList.Count > baseAreaWidth;
        float step = 0f;
        if (!tooManyCards)
        {
        ...
        {
            currentPosition = -(baseAreaWidth - gap) / 2;
            step = (baseAreaWidth - gap - cardWidth) / (cardList.Count - 1); // step * count + (width - step) = area - gap
        }
    ...
    private void SetCardSize(GameObject card)
    {
        ...
        float gap = 4f; // 上下gap
        float realCardHeight = baseAreaHeight - gap;
